...
public class AstalApps.Apps : Object {
    ...
    internal List<weak Application> query(string? search = "", SearchAlgorithm alg = FUZZY) {
        ...
        if (search.length == 1) {
            foreach (var app in list) {
                if (score(search, app, alg) == 0) arr.remove(app);
            }
        ...
        // filter
        foreach (var app in list) {
            if (score(search, app, alg) < min_score) arr.remove(app);
        }
        ...
        arr.sort_with_data((a, b) => {
            ...
            if (s1 == s2) return (int)b.frequency - (int)a.frequency;
    ...
    public void reload() {
        ...
        foreach (var app in arr) {
            if (!show_hidden && !app.should_show()) continue;
            ...
            a.notify.connect((pspec) => {
                if (pspec.name != "frequency") return;
    ...
    private void cache() {
        ...
        try {
            if (!FileUtils.test(cache_directory, FileTest.EXISTS)) File.new_for_path(cache_directory).make_directory_with_parents(null);
