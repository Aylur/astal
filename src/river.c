...
G_DEFINE_TYPE_WITH_CODE (AstalRiverRiver, astal_river_river, G_TYPE_OBJECT,
                         ...
                         G_IMPLEMENT_INTERFACE (G_TYPE_INITABLE, astal_river_river_initable_iface_init)
                         G_IMPLEMENT_INTERFACE (JSON_TYPE_SERIALIZABLE, astal_river_river_json_serializable_iface_init))
...
static guint astal_river_river_signals[ASTAL_RIVER_RIVER_N_SIGNALS] = {0,};
static GParamSpec *astal_river_river_properties[ASTAL_RIVER_RIVER_N_PROPERTIES] = {NULL,};
...
static void astal_river_river_get_property(GObject *object, guint property_id, GValue *value, GParamSpec *pspec) {
...
static void river_seat_status_handle_mode(void* data,
                                          struct zriver_seat_status_v1* seat_status,
                                          const char* mode) {
...
static void global_registry_handler(void* data,
    ...
    if (strcmp(interface, wl_output_interface.name) == 0) {
        ...
        AstalRiverOutput *output = astal_river_output_new(id, wl_out, priv->river_status_manager, priv->display);
...
static gboolean astal_river_river_initable_init (GInitable *initable, GCancellable *cancellable, GError **error) {
    ...
    if (priv->wl_source == NULL) {
        g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_FAILED, "Can not connect to wayland display");
        return FALSE;
    }
    ...
    if (priv->river_status_manager == NULL) {
        g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_FAILED, "Can not connect river status protocol");
        return FALSE;
    }

    priv->river_seat_status = zriver_status_manager_v1_get_river_seat_status(
      priv->river_status_manager, priv->seat);
    zriver_seat_status_v1_add_listener(
      priv->river_seat_status, &river_seat_status_listener, self);
...
static void astal_river_river_class_init(AstalRiverRiverClass *class) {
    ...
    astal_river_river_properties[ASTAL_RIVER_RIVER_PROP_MODE] =
        g_param_spec_string("mode",
                            ...
                            NULL,
                            G_PARAM_READABLE);
    astal_river_river_properties[ASTAL_RIVER_RIVER_PROP_FOCUSED_VIEW] =
        g_param_spec_string("focused-view",
                            "focused-view",
                            "currently focused view",
                            NULL,
                            G_PARAM_READABLE);
    astal_river_river_properties[ASTAL_RIVER_RIVER_PROP_FOCUSED_OUTPUT] =
        g_param_spec_string("focused-output",
                            "focused-output",
                            "currently focused-output",
                            NULL,
                            G_PARAM_READABLE);
    astal_river_river_properties[ASTAL_RIVER_RIVER_PROP_OUTPUTS] =
        g_param_spec_pointer("outputs",
                             "outputs",
