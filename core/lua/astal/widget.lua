...
local function astalify(ctor)
	...
	--- idk wher else to put this
	local widget_keybinds = {}

	function ctor:keybind(modifiers, key, callback)
		local keyval
		if key == "Escape" then
	...
	local function handle_keypress_event(self, event)
	...
	local id = ctor["on_key_press_event"]:connect(handle_keypress_event)

	ctor.on_destroy = function()
		GObject.signal_handler_disconnect(ctor, id)
	end
